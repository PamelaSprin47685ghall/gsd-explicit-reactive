...
- Task wave 不写入 task-plan frontmatter；每个 slice 恰好使用一个 sidecar：`.gsd/milestones/<M>/slices/<S>/<S>-TASK-WAVES.md`。
...
|---|---|---|---|---|
| 初次计划提示词 | 确认标准 plan prompt 与插件 sidecar prompt 一起发出 | `node --test --test-name-pattern "initial plan-slice" tests/reactive-dispatch.test.mjs` | `Plugin overlay: explicit task waves` | prompt 保留标准 `buildPlanSlicePrompt` 内容，并要求创建唯一 `Sxx-TASK-WAVES.md`；明确禁止 `wave` frontmatter |
