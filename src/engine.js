...
const resolveCreateSessionFactory = () => {
  ...
  if (!patchedCreateAgentSession) {
    throw new Error(
      '[DAG] createAgentSession not initialized. Ensure setupPatches() was called.',
    )
  }
...
const disableOfficialRules = (rules) => {
  rules
    .filter((r) => r.name?.includes('reactive-execute') && !r._dagDisabled)
    .forEach((r) => {
      r._dagDisabled = true
  ...
  rules
    .filter((r) => isPrimaryExecuteTaskRule(r.name) && !r._dagDisabled)
    .forEach((r) => {
      r._dagDisabled = true
      r._dagOriginalName = r.name
      r.match = async () => null
...
const registerDagRule = (rules, dagRule) => {
  if (!rules.some((r) => r.name === dagRule.name)) {
    const reactiveIdx = rules.findIndex((r) =>
      r.name?.includes('reactive-execute'),
    )
    if (reactiveIdx >= 0) {
      rules.splice(reactiveIdx, 0, dagRule)
    } else {
      const execIdx = rules.findIndex(
        (r) => r.name?.includes('executing →') || r.name?.includes('executing'),
      )
...
const executeDagTool = async (
  ...
  if (!payload)
    return {
      content: [
        {
          type: 'text',
          text: 'DAG payload expired or missing. Try re-dispatching the task.',
        },
      ],
      details: { unitId: _params.unitId, error: 'payload_not_found' },
    }
  ...
  const {
    ...
    db,
    dagWidget,
    dagTaskManagers: payloadDagTaskManagers,
  } = payload
  ...
  ctx?.ui?.notify?.(
    `[DAG] createSessionFactory=${createSessionFactory === patchedCreateAgentSession ? 'patched' : 'original'}`,
    'info',
  )
  ...
  if (!createSessionFactory) {
    return {
      content: [
        {
          type: 'text',
          text: 'DAG initialization failed: createAgentSession unavailable',
        },
      ],
  ...
  try {
    ...
    ctx?.ui?.notify?.(
      `DAG completed ${result.completed.length}/${result.total} tasks.`,
      'success',
    )
    return {
      content: [
        {
          type: 'text',
          text: `All DAG tasks completed (${result.completed.length}/${result.total}).`,
        },
      ],
      details: { completed: result.completed, total: result.total },
    }
  } catch (err) {
    ctx?.ui?.notify?.(`[DAG] Execution failed: ${err.message}`, 'error')

    // Dispatch standard stop signal so Guardian's notification listener
    // kicks off repair flow on the main session.
    try {
      ctx?.ui?.notify?.('dispatch-stop', 'error')
    } catch {}

    // Yield to event loop so Guardian's async notification handler has time
    // to queue its repair prompt before the session is aborted below.
    await new Promise((r) => setTimeout(r, 1000))

    // Abort the main session (masquerading as user cancellation) to cut through
    // GSD's native tool-retry machinery, letting Guardian's repair flow take over.
    if (ctx?.session?.abort) {
      ctx.session.abort(new Error('Operation aborted'))
    }

    return {
      content: [{ type: 'text', text: `DAG halted: ${err.message}` }],
      details: { error: 'dag_fatal', message: err.message },
    }
  }
}
...
export function injectExplicitDagEngine(
  ...
  if (!autoDispatch?.DISPATCH_RULES) {
    sessionCtx?.ui?.notify?.(
      '[DAG] DISPATCH_RULES not found in auto-dispatch module',
      'error',
    )
    return
  }
  ...
  const existingDagRule = rules.find(
    (r) => r.name === 'executing → dag (reactive-execute)',
  )
  if (existingDagRule) {
  ...
  const dagRule = {
    ...
    match: async (ctx) =>
      executeDagRule(ctx, core, autoDispatch, dagWidgets, dagTaskManagers),
  }
  ...
  sessionCtx?.ui?.notify?.(
    `[DAG] Injected dispatch rule (total: ${rules.length} rules)`,
    'info',
  )
  ...
  if (ruleRegistry?.initRegistry && ruleRegistry?.convertDispatchRules) {
    try {
      ...
      sessionCtx?.ui?.notify?.(
        `[DAG] Registry sync failed: ${err.message}`,
        'warning',
      )
    }
  }
...
async function executeDagRule(
  ...
  if (!ctx.state.activeSlice) {
    return {
      action: 'stop',
      reason:
        'DAG dispatch blocked: current phase is "executing" but activeSlice is missing. Run /gsd doctor and re-derive state before retrying.',
      level: 'error',
    }
  }
  ...
  if (ready.length === 0) {
    ...
    persistLatestError(
      ...
      mid,
      sid,
      `Deadlock detected: no ready tasks but ${incomplete.length} incomplete: [${incomplete.join(', ')}]. Check for missing dependencies or circular references.`,
      deps,
      ctx,
    )
    return backToPlanWithError(ctx, autoDispatch)
  ...
  const key = `${mid}/${sid}`
  const { totalTasks, averageWidth, criticalPathLength } =
    calculateDagMetrics(deps)
  ...
  if (totalTasks >= 3 && averageWidth < 1.5 && !width1Warned.has(key)) {
    ...
    persistLatestError(
      ...
      mid,
      sid,
      `DAG average concurrency width is ${averageWidth.toFixed(2)} (< 1.5). Please restructure dependencies to allow more parallel execution.`,
      deps,
      ctx,
    )
    return backToPlanWithError(ctx, autoDispatch)
  ...
  ctx?.ui?.notify?.(
    `[DAG] Starting parallel execution for ${ready.length} tasks: ${ready.join(', ')}`,
    'info',
  )
  emitDagDispatchLog(ctx, {
    event: 'dispatch',
  ...
  for (const taskId of ready) {
    try {
      ...
      ctx?.ui?.notify?.(
        `[DAG] Failed to mark ${taskId} as in_progress: ${err.message}`,
        'warning',
      )
    }
  }
  ...
  const readTaskPlans = (basePath, mid, sid, tasks) => {
    ...
    for (const t of tasks) {
      ...
      try {
        plans[t.id] = readFileSync(path, 'utf-8')
      } catch {}
    }
    return plans
  }
  ...
  payloadStore.set(
    unitId,
    { deps, allTasks: tasks, contextToolkit, db, dagWidget, dagTaskManagers },
    600000,
  )
